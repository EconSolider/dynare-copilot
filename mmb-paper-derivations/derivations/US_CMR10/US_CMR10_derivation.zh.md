...
例如，实现交叉检查记录了 \(\lambda_{f,t},\pi_t^{target},x_t^b,\mu_{\Upsilon,t},\chi_t,g_t,\mu_{z,t}^*,\gamma_t,\epsilon_t,\sigma_t,\zeta_{i,t},\tau_t^{oil}\) 的过程。
...
- 变量按随机中性技术趋势 \(z_t^*\) 和确定性投资专用技术趋势 \(\Upsilon^t\) 缩放。由于投资专用技术进步，资本和投资增长快于消费/产出。
- 平衡增长缩放为：
...
- 资本、投资、消费、政府支出、货币、信贷和净值使用 \(z_t^*\)、\(P_t\) 和 \(\Upsilon^t\) 转换为平稳变量。
...
| 内生 | `hU`, `wstarU`, \(H_t,w_t^*\) | 劳动与工资重设对象 | (F20)-(F22) |
...
| 外生 | `e_pitargetU`, \(\varepsilon_{\pi^*,t}\) | 通胀目标创新 | (F36) |
...
| 外生 | `e_muzstarU`, \(\varepsilon_{\mu_z^*,t}\) | 永久技术创新 | (F36) |
