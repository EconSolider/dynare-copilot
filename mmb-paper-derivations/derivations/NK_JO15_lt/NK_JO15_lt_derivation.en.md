...
- **Form**: log-linear `model(linear)`. Lowercase variables denote percentage deviations from deterministic steady state, except the implementation-level levels/indices such as $p_t$, $p_t^*$, $e_t$, $s_t$, and $q_t$ are linearized log variables. All steady-state deviations are zero.
...
$$(F5)\qquad
P_t^* C_t^*+E_t(Q_{t,t+1}^*D_{t+1}^{n*})\leq D_t^{n*}+W_t^*N_t^*+TR_t^*.
$$
...
$$(F9)\qquad
C_{H,t}^*=\alpha\left(\frac{P_{H,t}^*}{P_t^*}\right)^{-\eta}C_t^*,\quad
...
$$(F12)\qquad
Y_t(h)=A_tN_t(h),\quad Y_t^*(f)=A_t^*N_t^*(f).
$$
...
Y_t^*(f)=\left(\frac{P_t^*(f)}{P_{F,t}^*}\right)^{-\varepsilon}Y_t^*.
$$

Aggregate employment and production satisfy:

$$(F14)\qquad
N_t=\frac{Y_tD_t}{A_t},\quad N_t^*=\frac{Y_t^*D_t^*}{A_t^*},
$$

and, to first order around the deterministic steady state:

$$(F15)\qquad
y_t=a_t+n_t,\quad y_t^*=a_t^*+n_t^*.
$$
...
MC_{F,t}^*=\frac{P_t^*}{P_{F,t}^*}\frac{(1-\tau)(C_t^*)^\sigma (N_t^*)^\varphi R_t^*}{A_t^*}.
$$
...
c_t^*=E_t(c_{t+1}^*)-\frac{1}{\sigma}\{r_t^*-E_t(\pi_{t+1}^*)\}.
$$
...
\pi_{F,t}^*=\beta E_t(\pi_{F,t+1}^*)+\kappa_F mc_{F,t}^*,
$$
...
$$(F23)\qquad
x_t\equiv y_t-\bar y_t,\quad x_t^*\equiv y_t^*-\bar y_t^*.
$$

Natural output under flexible prices and constant marginal cost:

$$(F24)\qquad
\bar y_t=\frac{\varsigma\psi}{\delta}a_t-\frac{\omega_2\sigma\psi}{\delta}a_t^*,\quad
...
$$(F26)\qquad
x_t^*=E_t(x_{t+1}^*)-\frac{1}{\sigma_\omega}\{r_t^*-E_t(\pi_{F,t+1}^*)\}
\frac{\omega_2}{\omega_2+1}\{E_t(x_{t+1})-x_t\}
\frac{1}{\sigma_\omega}\bar r_t^*.
$$
...
$$(F27)\qquad
\bar r_t=-\Theta a_t-\Omega_1a_t^*,\quad
\bar r_t^*=-\Theta a_t^*-\Omega_1a_t.
$$

Marginal cost in output gaps:

$$(F28)\qquad
mc_{H,t}=\frac{\varsigma}{\omega_4+1}x_t+\frac{\omega_2\sigma}{\omega_4+1}x_t^*+r_t,\quad
...
$$(F29)\qquad
\pi_{H,t}=\beta E_t(\pi_{H,t+1})+\kappa_\omega(x_t+x_t^*)+r_t,\quad
\pi_{F,t}^*=\beta E_t(\pi_{F,t+1}^*)+\kappa_\omega(x_t^*+x_t)+r_t^*.
$$

`needs_review`: the MMB `.mod` implements the domestic and foreign PPI equations with separate coefficients on $x_t$ and $x_t^*$, consistent with combining (F22) and (F28), rather than the single $\kappa_\omega(x_t+x_t^*)$ coefficient written in the OCR source.
...
$$(F30)\qquad
Y_t(h)=C_t(h)+C_t^*(h),\quad Y_t^*(f)=C_t(f)+C_t^*(f).
$$
...
\pi_t^*=\pi_{F,t}^*-\alpha(s_t-s_{t-1}).
$$

Complete-markets UIP:

$$(F33)\qquad
r_t-r_t^*=E_t(e_{t+1})-e_t.
$$

International risk sharing:

$$(F34)\qquad
C_t=\vartheta C_t^*Q_t^{1/\sigma}.
$$
...
$$(F36)\qquad
c_t=c_t^*+\frac{1-2\alpha}{\sigma}s_t.
$$
...
$$(F38)\qquad
y_t=x_t+\bar y_t,\quad y_t^*=x_t^*+\bar y_t^*.
$$
...
`needs_review`: MinerU OCR for the source equation equivalent to (F39)-(F40) displays an extra $r_t$ and $r_t^*$ term in the CPI-inflation formulas. The appendix Dynare code and MMB implementation omit those terms.

Price, nominal exchange rate, real exchange rate, and terms-of-trade identities:

$$(F41)\qquad
p_t=p_{t-1}+\pi_t,\quad p_t^*=p_{t-1}^*+\pi_t^*.
$$
...
$$(F44)\qquad
r_t^*=\varrho^* r_{t-1}^*+(1-\varrho^*)(\phi_\pi^*\pi_t^*+\phi_x^*x_t^*)+m_t^*.
$$

Productivity processes:

$$(F45)\qquad
a_t=\rho a_{t-1}+\xi_t,\quad a_t^*=\rho^*a_{t-1}^*+\xi_t^*.
$$

The MMB simulation for `NK_JO15_lt` shocks the foreign productivity innovation $\xi_t^*$ with stderr 1. Monetary policy innovations $m_t,m_t^*$ and domestic productivity innovation $\xi_t$ are declared but not shocked in the checked implementation example.
...
$$(F47)\qquad
R=R^*=\beta^{-1},\quad MC_H=MC_F^*=\frac{\varepsilon-1}{\varepsilon}.
$$

$$(F48)\qquad
C=C^*,\quad Q=1,\quad S=1,\quad Y=C=Y^*=C^*.
$$
...
$$(F50)\qquad
\rho=\rho^*=0.55,\quad \phi_\pi=\phi_\pi^*=1.5,\quad
\phi_x=\phi_x^*=0.5,\quad \varrho=\varrho^*=0.4.
$$
...
- There is no capital stock in this model. Stock-timing concerns are limited to predetermined nominal interest rates and price levels: $r_{t-1}$ and $r_{t-1}^*$ enter Taylor rules; $p_{t-1}$ and $p_{t-1}^*$ enter price-level accumulation; lagged output gaps and lagged productivity enter CPI inflation identities.
...
| Endogenous | $x_t^*$ / `x_star` | foreign output gap | (F23), (F26), (F44) |
| Endogenous | $\pi_{F,t}^*$ / `pi_F_star` | foreign PPI inflation | (F22), (F29) |
| Endogenous | $r_t^*$ / `r_star` | foreign nominal interest-rate deviation | (F44) |
| Endogenous | $\pi_t^*$ / `pi_star` | foreign CPI inflation | (F32), (F40) |
| Endogenous | $\bar r_t$ / `r_bar` | domestic natural real interest rate | (F27) |
| Endogenous | $\bar r_t^*$ / `r_bar_star` | foreign natural real interest rate | (F27) |
| Endogenous | $a_t$ / `a` | domestic productivity deviation | (F45) |
| Endogenous | $a_t^*$ / `a_star` | foreign productivity deviation | (F45) |
| Endogenous | $mc_{H,t}$ / `mc` | domestic real marginal cost | (F28) |
| Endogenous | $mc_{F,t}^*$ / `mc_star` | foreign real marginal cost | (F28) |
| Endogenous | $y_t$ / `y` | domestic output deviation | (F38) |
| Endogenous | $y_t^*$ / `y_star` | foreign output deviation | (F38) |
| Endogenous | $\bar y_t$ / `y_bar` | domestic natural output | (F24) |
| Endogenous | $\bar y_t^*$ / `y_bar_star` | foreign natural output | (F24) |
| Endogenous | $p_t$ / `p` | domestic price level | (F41) |
| Endogenous | $p_t^*$ / `p_star` | foreign price level | (F41) |
...
| Exogenous | $m_t^*$ / `m_star` | foreign monetary policy innovation | (F44) |
| Exogenous | $\xi_t$ / `xi` | domestic productivity innovation | (F45) |
| Exogenous | $\xi_t^*$ / `xi_star` | foreign productivity innovation | (F45) |
...
| Parameter | $\rho,\rho^*$ / `rho`, `rho_star` | productivity persistence | (F45), (F50) |
| Parameter | $\phi_\pi,\phi_\pi^*,\phi_x,\phi_x^*$ / `phi_pi`, `phi_pi_star`, `phi_x`, `phi_x_star` | Taylor-rule responses | (F43), (F44) |
| Parameter | $\varrho,\varrho^*$ / `varrho`, `varrho_star` | interest-rate smoothing | (F43), (F44) |
