...
- **Goods**: home nontradable $N$, home tradable $H$, foreign nontradable $N^*$, foreign tradable $F$. Tradable goods are differentiated by country and cannot be price-discriminated inside the currency area.
...
The nonlinear source condition is $RER_t=P_t^*/P_t=\mu_t^*/\mu_t$.
...
$$
\pi_{F,t}-\varphi_F^*\pi_{F,t-1}
=\kappa_F^*(w_t^*-t_{F,t}^*-z_{T,t}^*)
+\beta(\pi_{F,t+1}-\varphi_F^*\pi_{F,t}).
$$
...
$$
\pi_t^*=\gamma^*(1-\lambda^*)\pi_{H,t}+\gamma^*\lambda^*\pi_{F,t}+(1-\gamma^*)\pi_{N,t}^*.
$$

- **(F25) EMU CPI inflation**:

$$
\pi_t^{EMU}=s\pi_t+(1-s)\pi_t^*.
$$
...
$$
t_{N,t}^*=t_{N,t-1}^*+\pi_{N,t}^*-\pi_t^*.
$$

- **(F32) Foreign tradable relative prices**:

$$
t_{T,t}^*=(1-\lambda^*)t_{H,t}^*+\lambda^*t_{F,t}^*,\quad
t_{H,t}^*=t_{H,t-1}^*+\pi_{H,t}-\pi_t^*,\quad
t_{F,t}^*=t_{F,t-1}^*+\pi_{F,t}-\pi_t^*.
$$
...
$$
rer_t=-(1-\gamma)(t_{N,t}-t_{T,t})+(1-\gamma^*)(t_{N,t}^*-t_{T,t}^*)+(1-\lambda-\lambda^*)(t_{H,t}-t_{F,t}).
$$
...
$$
y_{H,t}=(1-\eta)\left[\lambda c_{H,t}+(1-\lambda)c_{H,t}^*\right]+\eta g_{T,t}.
$$
...
$$
y_{F,t}^*=(1-\eta^*)\left[(1-\lambda^*)c_{F,t}+\lambda^*c_{F,t}^*\right]+\eta^*g_{T,t}^*.
$$
...
$$
y_t^*=\gamma^*(t_{F,t}^*+y_{F,t}^*)+(1-\gamma^*)(t_{N,t}^*+y_{N,t}^*).
$$
...
1. Set steady innovations to zero: $\varepsilon^m=\varepsilon^{Z}=\varepsilon^{ZT}=\varepsilon^{ZN}=\varepsilon^{ZT*}=\varepsilon^{ZN*}=\varepsilon^{GT}=\varepsilon^{GN}=\varepsilon^{GT*}=\varepsilon^{GN*}=0$.
...
4. Model-consistent observable constants are $\Delta p=\pi-\alpha^T$, $\Delta p^N=\pi-\alpha^N$, $\Delta p^*=\pi-\alpha^{T*}$, $\Delta p^{N*}=\pi-\alpha^{N*}$, $\Delta y=x+\alpha^T$, $\Delta y^N=x+\alpha^N$, $\Delta y^*=x+\alpha^{T*}$, $\Delta y^{N*}=x+\alpha^{N*}$, and $r=x+\pi-\log\beta$.
...
|---|---|---|---|
| Endogenous | $dy_t,dy_t^*$ | Home and foreign GDP growth | (F26), (F27) |
| Endogenous | $\mu_t,\mu_t^*$ | Marginal utility states | (F1)-(F4) |
| Endogenous | $r_t$ | Common nominal policy rate in linear implementation | (F1), (F40) |
| Endogenous | $\pi_t,\pi_t^*,\pi_t^{EMU}$ | Home, foreign and EMU CPI inflation | (F23)-(F25) |
| Endogenous | $c_t,c_t^*$ | Aggregate consumption | (F2), (F3), (F7)-(F12) |
| Endogenous | $rer_t$ | Real exchange rate | (F4), (F33) |
| Endogenous | $t_N,t_H,t_F,t_T,t_N^*,t_H^*,t_F^*,t_T^*$ | Relative prices | (F28)-(F33) |
| Endogenous | $c_H,c_F,c_N,c_H^*,c_F^*,c_N^*$ | Consumption by good origin/sector | (F7)-(F12), (F34)-(F37) |
| Endogenous | $l,l^*,w,w^*$ | Aggregate labor and wages | (F5), (F6), (F13), (F14) |
| Endogenous | $l_N,l_T,l_N^*,l_T^*$ | Sectoral labor | (F13)-(F18) |
| Endogenous | $y_N,y_H,y_N^*,y_F^*$ | Sectoral output | (F15)-(F18), (F34)-(F37) |
| Endogenous | $z_N,z_T,z_N^*,z_T^*$ | Productivity states | (F41)-(F44) |
| Endogenous | $\pi_N,\pi_H,\pi_N^*,\pi_F$ | Sectoral inflation | (F19)-(F25), (F30)-(F32) |
| Endogenous | $g_N,g_T,g_N^*,g_T^*$ | Sectoral demand/government spending states | (F45)-(F48) |
| Endogenous | $y,y^*$ | Real GDP aggregates | (F38), (F39) |
...
| $\varepsilon_t^{ZT*},\varepsilon_t^{ZN*}$ | Foreign tradable and nontradable technology innovations |
| $\varepsilon_t^{GT},\varepsilon_t^{GN}$ | Home tradable and nontradable demand/government-spending innovations |
| $\varepsilon_t^{GT*},\varepsilon_t^{GN*}$ | Foreign tradable and nontradable demand/government-spending innovations |
...
| $b,b^*$ | Habit persistence |
| $x$ | EMU aggregate real growth trend |
| $\alpha_T,\alpha_N,\alpha_T^*,\alpha_N^*$ | Sector/country productivity trends |
...
| $\gamma,\gamma^*$ | Tradable weights in home and foreign consumption baskets |
| $\lambda,\lambda^*$ | Home/foreign bias weights in tradable baskets |
| $s$ | Home country weight in EMU |
| $\bar w,\bar w^*$ | Labor disutility scale in linear implementation |
| $\varpi$ | Inverse labor-supply elasticity in paper notation |
| $\theta_N,\theta_H,\theta_N^*,\theta_F^*$ | Calvo non-reoptimization probabilities |
| $\varphi_N,\varphi_H,\varphi_N^*,\varphi_F^*$ | Inflation indexation shares |
| $\eta,\eta^*$ | Government-spending shares |
| $\rho_r,\gamma_\pi$ | Monetary policy smoothing and inflation response |
| $\rho_{ZT},\rho_{ZN},\rho_{ZT}^*,\rho_{ZN}^*$ | Technology shock persistence |
| $\rho_{GT},\rho_{GN},\rho_{GT}^*,\rho_{GN}^*$ | Demand/government-spending shock persistence |
