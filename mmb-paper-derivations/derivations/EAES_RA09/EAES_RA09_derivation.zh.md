...
- **商品**：本国不可贸易品 $N$、本国可贸易品 $H$、外国不可贸易品 $N^*$、外国可贸易品 $F$。可贸易品按国家差异化，并且在货币区内不能价格歧视。
...
非线性来源条件为 $RER_t=P_t^*/P_t=\mu_t^*/\mu_t$。
...
1. 将稳态创新设为零：$\varepsilon^m=\varepsilon^{Z}=\varepsilon^{ZT}=\varepsilon^{ZN}=\varepsilon^{ZT*}=\varepsilon^{ZN*}=\varepsilon^{GT}=\varepsilon^{GN}=\varepsilon^{GT*}=\varepsilon^{GN*}=0$。
2. 将规范化生产率与需求状态设在均值：$\tilde Z^i=1$ 且 $\tilde G^i$ 为常数。
3. 部门 $i$ 中数量变量的部门平衡增长率为 $(1+x)(1+\alpha^i)$。
4. 模型一致的可观测变量常数为 $\Delta p=\pi-\alpha^T$、$\Delta p^N=\pi-\alpha^N$、$\Delta p^*=\pi-\alpha^{T*}$、$\Delta p^{N*}=\pi-\alpha^{N*}$、$\Delta y=x+\alpha^T$、$\Delta y^N=x+\alpha^N$、$\Delta y^*=x+\alpha^{T*}$、$\Delta y^{N*}=x+\alpha^{N*}$ 以及 $r=x+\pi-\log\beta$。
...
| 类别 | 符号 | 含义 | 主要方程 |
|---|---|---|---|
| 内生 | $dy_t,dy_t^*$ | 本国和外国 GDP 增长 | (F26), (F27) |
| 内生 | $\mu_t,\mu_t^*$ | 边际效用状态 | (F1)-(F4) |
| 内生 | $r_t$ | 线性实现中的共同名义政策利率 | (F1), (F40) |
| 内生 | $\pi_t,\pi_t^*,\pi_t^{EMU}$ | 本国、外国和 EMU CPI 通胀 | (F23)-(F25) |
| 内生 | $c_t,c_t^*$ | 总消费 | (F2), (F3), (F7)-(F12) |
| 内生 | $rer_t$ | 实际汇率 | (F4), (F33) |
| 内生 | $t_N,t_H,t_F,t_T,t_N^*,t_H^*,t_F^*,t_T^*$ | 相对价格 | (F28)-(F33) |
| 内生 | $c_H,c_F,c_N,c_H^*,c_F^*,c_N^*$ | 按来源/部门划分的消费 | (F7)-(F12), (F34)-(F37) |
| 内生 | $l,l^*,w,w^*$ | 总劳动和工资 | (F5), (F6), (F13), (F14) |
| 内生 | $l_N,l_T,l_N^*,l_T^*$ | 部门劳动 | (F13)-(F18) |
| 内生 | $y_N,y_H,y_N^*,y_F^*$ | 部门产出 | (F15)-(F18), (F34)-(F37) |
| 内生 | $z_N,z_T,z_N^*,z_T^*$ | 生产率状态 | (F41)-(F44) |
| 内生 | $\pi_N,\pi_H,\pi_N^*,\pi_F$ | 部门通胀 | (F19)-(F25), (F30)-(F32) |
| 内生 | $g_N,g_T,g_N^*,g_T^*$ | 部门需求/政府支出状态 | (F45)-(F48) |
| 内生 | $y,y^*$ | 实际 GDP 加总 | (F38), (F39) |
...
| $\varepsilon_t^{ZT*},\varepsilon_t^{ZN*}$ | 外国可贸易和不可贸易技术创新 |
| $\varepsilon_t^{GT},\varepsilon_t^{GN}$ | 本国可贸易和不可贸易需求/政府支出创新 |
| $\varepsilon_t^{GT*},\varepsilon_t^{GN*}$ | 外国可贸易和不可贸易需求/政府支出创新 |
...
| $b,b^*$ | 习惯持续性 |
| $x$ | EMU 总实际增长趋势 |
| $\alpha_T,\alpha_N,\alpha_T^*,\alpha_N^*$ | 部门/国家生产率趋势 |
| $\varepsilon$ | 可贸易品与不可贸易品之间的替代弹性 |
| $\nu$ | 本国与外国可贸易品之间的替代弹性 |
| $\sigma$ | 品种间替代弹性 |
| $\gamma,\gamma^*$ | 本国和外国消费篮子中的可贸易品权重 |
| $\lambda,\lambda^*$ | 可贸易品篮子中的本国/外国偏好权重 |
| $s$ | 本国在 EMU 中的权重 |
| $\bar w,\bar w^*$ | 线性实现中的劳动负效用尺度 |
| $\varpi$ | 论文符号中的劳动供给弹性倒数 |
| $\theta_N,\theta_H,\theta_N^*,\theta_F^*$ | Calvo 不重新定价概率 |
| $\varphi_N,\varphi_H,\varphi_N^*,\varphi_F^*$ | 通胀指数化份额 |
| $\eta,\eta^*$ | 政府支出份额 |
| $\rho_r,\gamma_\pi$ | 货币政策平滑和通胀反应 |
| $\rho_{ZT},\rho_{ZN},\rho_{ZT}^*,\rho_{ZN}^*$ | 技术冲击持续性 |
| $\rho_{GT},\rho_{GN},\rho_{GT}^*,\rho_{GN}^*$ | 需求/政府支出冲击持续性 |
