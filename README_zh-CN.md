...
>
> 它的核心在于把所有经验丰富的建模者无论如何都会遵循的流程**固定下来**：先推导方程、后翻译代码、后阶段运行验证。时序错误、命名冲突、稳态代数失误，在它们成为错误结果前就被截住。内置**双层参考库**——来自 [Macroeconomic Model Data Base](https://www.macromodelbase.com/rep-mmb) 的 **149 篇 MMB 复制模型**（用于经济结构参照）+ 来自 [DSGE_mod](https://github.com/JohannesPfeifer/DSGE_mod) 的 **41 个 Pfeifer 编程示例**（用于 Dynare 命令/块写法参照）——每个新模型都从经过验证的参照库启动，而不是空白纸。每次完成的模型自动归档进**个人记忆库**，下次建模会自动调用，越用越自。所有 IRF 以顶刊级矢量图交付，可以直接粘进论文。
>
...
- **模型参考库**（`references/examples/`，由 `references/catalog.csv` 索引）：149 篇 MMB rep-mmb 复制模型（每篇一个 `.mod`，文件名即其 `ModelID`），加一个最小教学范例（基础 RBC，推导与 `.mod` 逐行对应）。
...
建模时先检索两套本地库，再查个人 `memory-catalog.csv`，仅在两库都无命中时才上网补充论文特定细节（精确校准来源、某条推导）。**DSGE_mod 已完全本地化，无需上网查找。**
...
| 复制某论文模型、"要个带 X 特征的模型"、不确定有无现成实现 | 本地双库检索（首先运行）         | grep `catalog.csv`（结构）+ `catalog-code.csv`（语法）→ `memory-catalog.csv` → web |
...
4. **运行-纠错闭环**——接好 MATLAB MCP 时自动跑 Dynare、读报错、做最小修复、重跑。
5. **本地双库检索**——建模前先查两套独立的本地库：
   ...
   两套本地库之后，再查你**个人记忆库**（`memory-catalog.csv`），最后才上网补论文特定细节（精确校准来源、某条推导步骤）。**DSGE_mod 已全部本地化，不再需要上网查找。** 记忆库在每次建模任务结束时自动更新——最终 `.mod` 与推导 md 归档到 `references/memory/`，并在 `references/memory-catalog.csv` 里新增一条索引。线性化参照只取方程内容/时序/校准，不照搬代码形式。
...
plugins/dynare-mod/                  # 插件
  ...
  └── skills/dynare-mod/             # 捆绑的 skill
      ...
      └── references/                # 按需读取的细节文件 + 模型索引 + 绘图脚本
          ...
          ├── memory-catalog.csv     # 你历次任务积累的模型索引（随使用增长）
...
| `catalog-lookup.md`        | 如何按特征检索两套 catalog、各自的分类索引、记忆库检索说明，以及用参考 `.mod` 的注意事项（线性化版 vs 非线性、参照不等于成品）                               |
| `memory-catalog.csv`       | 你历次任务积累的模型索引，字段格式与 catalog.csv 兼容，另含 `Task` 和 `DateAdded` 列；每次建模任务结束时自动追加                                          |
| `examples/`                | 149 篇 MMB rep-mmb 复制 `.mod`（文件名即 `ModelID`），加最小 RBC 教学范例（八节推导 + 配套 `.mod`，FOC 编号与 `[name=]` 一一对应）。回答"经济结构该怎么设" |
...
| `memory/`                  | 你历次会话产出的 `.mod` 与推导 md 存档；未来建模任务自动优先检索，早于网络搜索                                                                             |
