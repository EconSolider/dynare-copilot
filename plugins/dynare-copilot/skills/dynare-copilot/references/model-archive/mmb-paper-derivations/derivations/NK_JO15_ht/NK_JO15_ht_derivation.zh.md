...
| `x_star` | $x_t^{*}$ | 外国产出缺口 | (F13), (F20) |
| `pi_F_star` | $\pi_{F,t}^{*}$ | 外国 PPI 通胀 | (F15) |
| `r_star` | $\hat r_t^{*}$ | 外国名义利率偏离 | (F24) |
| `pi_star` | $\pi_t^{*}$ | 外国 CPI 通胀 | (F19) |
| `r_bar` | $\bar r_t$ | 本国自然实际利率 | (F21) |
| `r_bar_star` | $\bar r_t^{*}$ | 外国自然实际利率 | (F21) |
| `a` | $a_t$ | 本国生产率偏离 | (F23) |
| `a_star` | $a_t^{*}$ | 外国生产率偏离 | (F23) |
| `mc` | $mc_{H,t}$ | 本国实际边际成本 | (F14) |
| `mc_star` | $mc_{F,t}^{*}$ | 外国实际边际成本 | (F14) |
| `y` | $y_t$ | 本国产出偏离 | (F17), (F20) |
| `y_star` | $y_t^{*}$ | 外国产出偏离 | (F17), (F20) |
| `y_bar` | $\bar y_t$ | 本国自然产出 | (F20) |
| `y_bar_star` | $\bar y_t^{*}$ | 外国自然产出 | (F20) |
| `p` | $p_t$ | 本国 CPI 价格水平偏离 | (F22) |
| `p_star` | $p_t^{*}$ | 外国 CPI 价格水平偏离 | (F22) |
| `e` | $e_t$ | 名义汇率对数偏离 | (F22) |
| `s` | $s_t$ | 贸易条件 | (F18) |
| `q` | $q_t$ | 实际汇率 | (F18) |
...
| `rho_star` | $\rho^{*}$ | 外国生产率持续性 | 0.55 |
| `phi_pi` | $\phi_{\pi}$ | 本国通胀反应 | 1.5 |
| `phi_pi_star` | $\phi_{\pi}^{*}$ | 外国通胀反应 | 1.5 |
| `phi_x` | $\phi_x$ | 本国产出缺口反应 | 0.5 |
| `phi_x_star` | $\phi_x^{*}$ | 外国产出缺口反应 | 0.5 |
| `varrho` | $\varrho$ | 本国利率平滑 | 0.4 |
| `varrho_star` | $\varrho^{*}$ | 外国利率平滑 | 0.4 |
