...
2. 设定校准目标：$s_I=0.25$、$s_G=0.18$、总进口份额 $s_M=0.12$、$\omega_C=0.052$、$\omega_I=0.36$、外国 $\omega^*_C=0.01$、外国 $\omega^*_I=0.07$、$\delta=0.025$、$\beta=0.997$、$\tau_K=0.30$。
3. 按实现计算稳态实际利率：
$$
...
| 内生 | $m^*_t$ | 外国进口 / 本国出口 | (F3)-(F12) 的外国对应式，(F24) |
...
| $\omega_C^*=0.01$, $\omega_I^*=0.07$ | 外国 DT 进口份额校准 |
