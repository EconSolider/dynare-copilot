...
- **形式**：对数线性 `model(linear)`。除实现中的价格/汇率类线性化对数变量 $p_t$、$p_t^*$、$e_t$、$s_t$ 和 $q_t$ 外，小写变量表示相对确定性稳态的百分比偏离。所有稳态偏离均为零。
...
`needs_review`：MMB `.mod` 使用的国内和国外 PPI 方程对 $x_t$ 和 $x_t^*$ 有不同系数，这与合并 (F22) 和 (F28) 一致，而不是 OCR 来源中写出的单一 $\kappa_\omega(x_t+x_t^*)$ 系数。
...
`needs_review`：MinerU OCR 中与 (F39)-(F40) 对应的来源方程在 CPI 通胀公式里显示了额外的 $r_t$ 和 $r_t^*$ 项。附录 Dynare 代码和 MMB 实现均省略这些项。
...
`NK_JO15_lt` 的 MMB 模拟对国外生产率创新 $\xi_t^*$ 设定 stderr 1。货币政策创新 $m_t,m_t^*$ 和国内生产率创新 $\xi_t$ 在已检查的实现示例中被声明但未施加冲击。
...
- 本模型没有资本存量。存量时序问题仅限于预定的名义利率和价格水平：$r_{t-1}$ 和 $r_{t-1}^*$ 进入 Taylor 规则；$p_{t-1}$ 和 $p_{t-1}^*$ 进入价格水平累积；滞后产出缺口和滞后生产率进入 CPI 通胀恒等式。
...
| 内生 | $x_t^*$ / `x_star` | 国外产出缺口 | (F23), (F26), (F44) |
| 内生 | $\pi_{F,t}^*$ / `pi_F_star` | 国外 PPI 通胀 | (F22), (F29) |
| 内生 | $r_t^*$ / `r_star` | 国外名义利率偏离 | (F44) |
| 内生 | $\pi_t^*$ / `pi_star` | 国外 CPI 通胀 | (F32), (F40) |
| 内生 | $\bar r_t$ / `r_bar` | 国内自然实际利率 | (F27) |
| 内生 | $\bar r_t^*$ / `r_bar_star` | 国外自然实际利率 | (F27) |
| 内生 | $a_t$ / `a` | 国内生产率偏离 | (F45) |
| 内生 | $a_t^*$ / `a_star` | 国外生产率偏离 | (F45) |
| 内生 | $mc_{H,t}$ / `mc` | 国内实际边际成本 | (F28) |
| 内生 | $mc_{F,t}^*$ / `mc_star` | 国外实际边际成本 | (F28) |
| 内生 | $y_t$ / `y` | 国内产出偏离 | (F38) |
| 内生 | $y_t^*$ / `y_star` | 国外产出偏离 | (F38) |
| 内生 | $\bar y_t$ / `y_bar` | 国内自然产出 | (F24) |
| 内生 | $\bar y_t^*$ / `y_bar_star` | 国外自然产出 | (F24) |
| 内生 | $p_t$ / `p` | 国内价格水平 | (F41) |
| 内生 | $p_t^*$ / `p_star` | 国外价格水平 | (F41) |
| 内生 | $e_t$ / `e` | 名义汇率 | (F42) |
| 内生 | $s_t$ / `s` | 贸易条件 | (F35), (F37) |
| 内生 | $q_t$ / `q` | 实际汇率 | (F35), (F42) |
| 外生 | $m_t$ / `m` | 国内货币政策创新 | (F43) |
| 外生 | $m_t^*$ / `m_star` | 国外货币政策创新 | (F44) |
| 外生 | $\xi_t$ / `xi` | 国内生产率创新 | (F45) |
| 外生 | $\xi_t^*$ / `xi_star` | 国外生产率创新 | (F45) |
...
| 参数 | $\rho,\rho^*$ / `rho`, `rho_star` | 生产率持续性 | (F45), (F50) |
| 参数 | $\phi_\pi,\phi_\pi^*,\phi_x,\phi_x^*$ / `phi_pi`, `phi_pi_star`, `phi_x`, `phi_x_star` | Taylor 规则反应系数 | (F43), (F44) |
| 参数 | $\varrho,\varrho^*$ / `varrho`, `varrho_star` | 利率平滑 | (F43), (F44) |
