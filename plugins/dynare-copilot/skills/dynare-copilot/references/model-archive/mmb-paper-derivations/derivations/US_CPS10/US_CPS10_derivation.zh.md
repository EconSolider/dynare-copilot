...
论文 OCR 注记：MinerU Markdown 中方程 (21) 在等式两边重复 $\log\pi_t^*$；预期的持久性目标过程应使用滞后目标，这与 Rep-MMB 实现和论文上下文一致。此归一化标为 `needs_review`。
...
| Exogenous shock | `pits` | 通胀目标创新 $\varepsilon_{\pi^*,t}$ | (F15) |
...
| Parameter | `rhopit` / $\rho_{\pi^*}$ | 通胀目标持久性；基准 0.995 | (F15) |
