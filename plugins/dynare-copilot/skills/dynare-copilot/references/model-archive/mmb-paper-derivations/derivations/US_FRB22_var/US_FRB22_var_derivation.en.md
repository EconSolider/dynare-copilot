# US_FRB22_var - Derivation (Optimization Problems + First-Order Conditions)
...
Provenance: `US_FRB22_var`, Brayton, Flint; Reifschneider, David (2022), "LINVER: The Linear Version of FRB/US", Finance and Economics Discussion Series 2022-053, DOI `10.17016/feds.2022.053`. Source Markdown: `raw/mmb_mineru/runs/us_frb22_mcap_us_frb22_mcapwp_us_frb22_mceall_us__linver_the_linear_version_of_frb_us__a8780356/full.md`. Raw PDF: `raw/mmb_papers/LINVER- The Linear Version of FRB:US.pdf`. MinerU run id: `a8780356-98b8-4931-bad6-3b2c5b4b2f0c`.

## 1. Model Overview

- **Model**: LINVER, the linear version of the Federal Reserve Board's FRB/US model, archived here as MMB model `US_FRB22_var`.
- **Variant**: The MMB implementation comment identifies this as the variant in which all expectations are based on VAR predictions. This is `implementation_cross_check`; the paper describes all-VAR expectations as one LINVER/FRB-US simulation option.
- **Purpose**: Large-scale U.S. semi-structural macroeconomic policy model for deterministic and stochastic simulations, including experiments with model-consistent (MC) expectations, VAR-based expectations, and the effective lower bound (ELB).
- **Agents/blocks**: Households and firms are represented through estimated expenditure, production, wage-price, financial-market, fiscal, external, and expectations blocks rather than through a compact DSGE set of optimizing agents. Monetary policy is represented by selectable simple rules and ELB/nonlinear policy constraints.
- **Form**: Linear model around 2018-2019 average conditions, with optional nonlinear constraints such as the ELB imposed by iterative additive adjustments. `needs_review`: the paper does not publish the full LINVER equation listing in this source.
...
- **Expectations formation**: Agents can use model-consistent expectations or expectations from limited-information VAR models. For `US_FRB22_var`, all expectations are VAR-based according to the implementation cross-check.
...
This records the paper's MC expectation concept for comparison with the all-VAR variant.
...
The paper lists this as one rule used in the rule-comparison simulations.
...
- The current archive variant is all VAR-based expectations according to the MMB implementation comment. The paper also discusses mixed and full MC expectation cases.
...
- `implementation_cross_check`: `raw/mmb/mmci-cli/models/US_FRB22_var/US_FRB22_var.mod` declares 276 endogenous variables, 114 exogenous entries, and 1393 parameters/coefficient names. It uses a plain `model;` block but is described by the paper and metadata as linearized. Dynare was not run.
...
| Endogenous | `rg5`, `rg10`, `rg30` | Treasury yields / term-premium channels | needs_review |
