...
其中 Foreign 进口关税 $\tau_t^{*}$ 作用于 Home 商品，债券计价遵循模型变体。US-China 扩展将债券计价改为美元；本归档条目不把该扩展重构为单独模型。
...
约束为完整均衡条件和货币政策规则。Foreign 关于 $\tau_t^{*}$ 的问题是对称的。这些关税 FOC 未在正文列出，因此需要对照在线附录 C 进行 needs_review。
...
1. 对称校准中设 $A=A^{*}=1$；US-China 应用中使用 $A^{*}/A=1/3$。
2. 设 $\pi_h=\pi_f^{*}=1$；在 PPI 规则下，当货币政策冲击为零时，$R=R^{*}=\beta^{-1}$。
3. 将净国外资产设为目标值，通常为 $b=\bar{b}=0$。
4. 给定关税 $\tau,\tau^{*}$，由 (F17)-(F18) 以及相应中间品价格指数计算 CPI/进口价格指数 $P,P^{*},P_x,P_x^{*}$。
...
- 使用生产者货币定价：黏性价格是国内生产者价格，因此 `ppi` 变体目标为 $\pi_{h,t}$ 和 $\pi_{f,t}^{*}$。
...
| 内生变量 | $C_t^{*}$ / `Cs` | Foreign 总消费 | (F9), (F10), (F18) |
| 内生变量 | $H_t,L_t$ / `L` | Home 劳动 | (F1), (F7) |
| 内生变量 | $H_t^{*},L_t^{*}$ / `Ls` | Foreign 劳动 | (F8), (F12) |
| 内生变量 | $Y_t,Y_t^{*}$ / `Y`, `Ys` | Home 和 Foreign 产出 | (F7), (F12), (F15), (F16) |
| 内生变量 | $X_t,X_t^{*}$ / `X`, `Xs` | 中间品 | (F6), (F12) |
| 内生变量 | $MC_t,MC_t^{*}$ / `Mc`, `Mcs` | 边际成本 | (F5), (F11) |
| 内生变量 | $\pi_{h,t},\pi_{f,t}^{*}$ / `Pih`, `Pif` | PPI 通胀率 | (F4), (F11), (F20), (F21) |
| 内生变量 | $P_t,P_t^{*}$ / `P`, `Ps` | CPI 价格指数 | (F17), (F18) |
| 内生变量 | $S_t$ / `S` | 贸易条件 / 相对价格 | (F2), (F9), (F17), (F18), (F19) |
| 内生变量 | $b_t$ / `nfa` | 净国外资产 | (F19) |
| 内生变量 | $R_t,R_t^{*}$ / `R`, `Rs` | 名义利率 | (F20), (F21) |
| 内生政策 | $\tau_t,\tau_t^{*}$ / `T`, `Ts` | 进口关税 | (F2), (F9), (F23) |
| 外生变量 | $\varepsilon^A_t,\varepsilon^{A^{*}}_t$ / `eA`, `eAs` | 生产率创新 | (F22) |
| 外生变量 | $\varepsilon^T_t,\varepsilon^{T^{*}}_t$ / `eT`, `eTs` | MMB 实现中的关税冲击 / 关税移位项 | (F23) |
| 外生变量 | $\varepsilon^R_t,\varepsilon^{R^{*}}_t$ / `epsR`, `epsRs`, `interest_` | 货币政策冲击 | (F20), (F21), (F23) |
...
| 参数 | $\phi,\phi^{*}$ / `phi` | Rotemberg 调价成本 | (F4), (F11), (F15), (F16) |
| 参数 | $\alpha$ / `alpha` | 中间品份额 | (F5), (F7), (F12) |
| 参数 | $\gamma_h,\gamma_f,\gamma_{xh},\gamma_{xf}$ / `gamh`, `gamf`, `gamxh`, `gamxf` | home-bias 权重 | (F17), (F18) |
| 参数 | $\mu_\pi,\mu_\pi^{*}$ / `mu` | 货币规则中的通胀反应系数 | (F20), (F21) |
| 参数 | $\rho_a,\rho_R,\rho_{R^{*}}$ / `rhoa`, `rhoR`, `rhoRs` | 冲击持久性 | (F22), (F23) |
