...
| 顶刊级 IRF 图、好看的图、导出 PDF 论文图、多情景/多冲击对比图、替换自带画图 | 发表级绘图 | `plot_irfs_pub.m` | `references/publication-plots.md` |
...
8. **收尾清理工作目录**：复查文件夹，**白名单内的中间产物直接删**（Dynare 自动生成的
   ...
   最终 .mod、**用户手写的 `<模型名>_steadystate.m`**（这是稳态文件不是中间产物）、
   **`plot_irfs_pub.m` 绘图脚本及导出的 `fig_*.pdf/.eps/.png` 发表图**（成果与 skill 资产）。详见
