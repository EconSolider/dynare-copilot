...
**建模/复制类任务（产出新 .mod）起手列 TodoList 时，必须把第7步「⏸ 询问用户是否归档模型」
作为固定的最后一条显式写进去**，并保持未勾选直到归档询问真正发出。原因很实在：终端「停下问用户」
的门控一旦只藏在交付清单深处，极易被用户一句"收尾/差不多了"带过而整步蒸发（这正是本 skill 反复
漏掉归档的根因）——把它做成一条从一开始就可见、始终待办的清单项，是防遗漏最可靠的机制。
...
第6步   交付：见下方"交付格式"。
第7步   ⏸🔒 归档门控（建模/复制类任务且产出了新 .mod 必做；纯排错、无新 .mod 产出则跳过）——
        **任务的最后一个动作，也是历史上最常被漏掉的一步**：把"是否把模型归档到存档库"作为交付
        消息的**固定收尾行**问出来（一行，是/否），然后停下等回答。
        **关键反例：用户说"收尾 / wrap up / 差不多了 / 可以了"绝不豁免本步**——"收尾"的意思是
        把任务正确收完，而正确收完恰恰**包含**这一行归档询问。它只花一句话，却让本次模型进库、
        供未来任务（第1.3步）自动检索复用；漏问的代价是这份价值**永久且无声地丢失**，没有任何
        补救信号。具体按 `references/model-archive.md`「二、存档」执行：用户答"是"才复制
        `.mod` 与推导 md 到 `references/model-archive/` 并追加 catalog 索引行；答"否"则跳过文件操作。
...
9. **收尾清理工作目录**：复查文件夹，**白名单内的中间产物直接删**（Dynare 自动生成的
   ...
   报告删了哪些、保留了哪些。
10. ⏸🔒 **归档询问行（强制收尾，见第7步）**：这不是可选附加，而是交付消息的**固定最后一行**。
    无论用户此前是否说过"收尾/wrap up"，都要把"是否将本模型归档到存档库供未来复用？(是/否)"
    这一行问出来再结束本轮；用户答"是"后才按 `references/model-archive.md`「二、存档」复制文件、
    追加 catalog 行。**漏掉这一行 = 任务未完成**——它一句话的成本，换的是模型长期可检索复用。
